...
или ОНМК/ИМ), **ЖКТ / НЖБП**, **повторные пневмонии** (за 2 года).

**Структура анализа**

- **Этап 1.** Метаболический синдром → **частота осложнений**: сколько осложнений в каждой группе и
  **отношение шансов (OR)** «TyG ≥ 8,5 против TyG < 8,5», без поправки и с поправкой на возраст, пол, КТ.
...
freq.loc[mask4, 'q (FDR)'] = multipletests(freq.loc[mask4, 'p'], method='fdr_bh')[1]
...
sig = fdf['q (FDR)'].fillna(fdf['p']).astype(float).values < 0.05
...
выше, чем в группе сравнения. Если OR остаётся значимым после поправки на возраст, пол и тяжесть КТ, вклад
метаболического синдрома **независим** от этих факторов.""")
...
> (11 признаков × 4 осложнения = 44 теста в группе) контролируем поправкой Бенджамини–Хохберга: сплошная
> «звёздочка» — связь прошла FDR (q<0,05), «°» — номинально значима (p<0,05), но FDR не прошла. Признак
...
for g in GROUPS:
    ...
    # FDR по всем валидным p внутри группы (44 теста)
...
> Число событий на признак (EPV) невелико, поэтому надёжны только модели с ≥ ~50 событиями (в основной
> группе — ЖКТ, ССЗ/АГ, пневмонии); остальные — поисковые. Важности Gini сопоставимы **только внутри одной
...
группе сравнения — **возраст, ИМТ и тяжесть КТ**. Модели с AUC около 0,5 (повторные пневмонии в обеих
группах; СД в группе сравнения) неинформативны и в выводы не берутся.""")
...
for m in DYN:
    for g in GROUPS:
        ...
        dyn_rows.append({'Показатель': m, 'Группа': GROUP_FULL[g], 'n': n,
                         ...
                         'Δ медиана': d.median(), 'p (Вилкоксон)': p, '': star(p)})
    # межгрупповое сравнение Δ
    ...
    dyn_rows.append({'Показатель': m, 'Группа': 'Δ: сравнение групп (Манна–Уитни)', 'n': len(dlo) + len(dhi),
                     'Медиана баз': np.nan, 'Медиана 24м': np.nan,
                     'Δ медиана': np.nan, 'p (Вилкоксон)': pbtw, '': star(pbtw)})
dyn_tab = pd.DataFrame(dyn_rows)
print('Динамика TyG и HOMA-IR: острый период → 24 месяца. *** p<0,001, ** p<0,01, * p<0,05.')
display(dyn_tab.set_index(['Показатель', 'Группа']).round(3))
...
ССЗ/АГ, повторные пневмонии) остаётся значимым после поправки на возраст, пол, тяжесть КТ **и вакцинацию**
...
признаки (НОМА-IR, ИМТ, глюкоза, ТГ), тогда как в группе сравнения ведущими чаще оказываются возраст и
тяжесть КТ.
...
- **Новый СД в группе сравнения** — всего 4 события: столбец СД в матрице OR и в машинном обучении
  недостаточно обеспечен (AUC ниже 0,5), затемнён и в выводы не берётся; скорректированное OR для СД помечено
  как неустойчивое (†, широкий ДИ).
...
- **Повторные пневмонии**: модели около случайного угадывания в обеих группах — сильного метаболического
  предиктора нет; столбец счётный (0–3), бинаризован по «>0».
